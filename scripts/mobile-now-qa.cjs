...
(async()=>{
  ...
  const input=page.locator('.novus-panel-input input');await input.waitFor();
  assert.match(await input.inputValue(),/Life Score/);await input.press('Enter');await page.getByText('Local mobile QA response.').waitFor();await page.keyboard.press('Escape');await page.waitForTimeout(350);
