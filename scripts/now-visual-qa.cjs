...
(async () => {
  ...
  if (process.env.QA_MOTION) {
    ...
    console.log('reduced-motion renderer:', await page.locator('.now-liquid-field').getAttribute('data-renderer'));
  ...
  if (process.env.QA_SUITE) {
    ...
    assert.equal(await page.locator('.now-liquid-field').getAttribute('data-renderer'),'fallback');
    assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);
