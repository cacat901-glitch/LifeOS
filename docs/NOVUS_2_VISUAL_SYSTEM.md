...
| `src/components/novus/now-liquid-field.tsx` / `NowLiquidField` | Canonical WebGL material, timing, quality, fallback | Locked identity; React, browser WebGL, store; reuse cautiously because paths are composition-specific |
...
Each mounted eligible field owns its resources and RAF. Props are `pixelRatioCap=1.35`, `mobile=false`, `intelligence=false`, `activity=1`. Both Now JSX variants exist, but the effect’s 1024px guard activates only the correct renderer. Opening Novus releases Now’s active rendering resources and displays its fallback; the intelligence renderer takes priority. Closing reinitializes Now. `SystemAtmosphere` returns null on Now, avoiding a second background loop there; on other routes its Canvas2D field pauses while Novus opens.
