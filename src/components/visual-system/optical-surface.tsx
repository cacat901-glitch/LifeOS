...
export function OpticalSurface({ children, className, light = "upper", as: Tag = "section" }: {
  children: ReactNode;
  className?: string;
  light?: OpticalLight;
  as?: "section" | "article";
}) {
  return <Tag className={cn("target-instrument optical-surface", className)} data-optical-light={light}>{children}</Tag>;
}
